...
func SeedQuizData(db *gorm.DB, moduleTitles []string) {
    ...
    for _, title := range moduleTitles {
        ...
        if len(users) == 0 { log.Printf("SeedQuiz: no users to attach, skipping module '%s'", title); continue }
        ...
        for i := 0; i < needed; i++ {
            ...
            if err := db.Create(&q).Error; err != nil { log.Printf("SeedQuiz: create question err: %v", err) }
        }
